...
export const name = 'sample.isolated-extension-completion-provider-not-registered'
...
export const test: Test = async ({ Extension }) => {
  ...
  await expectAddWebExtensionToThrow(
    Extension,
    uri,
    'completion provider isolatedCompletionMissingRegistration is contributed in extension.json but not registered',
  )
}
