...
export const name = 'sample.isolated-extension-diagnostic-provider-not-registered'
...
export const test: Test = async ({ Extension }) => {
  ...
  await expectAddWebExtensionToThrow(
    Extension,
    uri,
    'diagnostic provider isolatedDiagnosticMissingRegistration is contributed in extension.json but not registered',
  )
}
