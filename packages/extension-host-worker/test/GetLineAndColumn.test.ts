...
test('getLineAndColumn - empty string', () => {
  ...
  const start = 0
  const end = 0
  expect(GetLineAndColumn.getLineAndColumn(text, start, end)).toEqual({ column: 0, line: 0 })
})
...
test('getLineAndColumn', () => {
  ...
  const start = 221
  const end = 286
  expect(GetLineAndColumn.getLineAndColumn(text, start, end)).toEqual({ column: 0, line: 3 })
})
