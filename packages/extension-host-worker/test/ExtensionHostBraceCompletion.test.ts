...
test('executeBraceCompletionProvider - error - result is undefined', async () => {
  ...
  await expect(ExtensionHostBraceCompletion.executeBraceCompletionProvider(1, 0)).rejects.toThrow(
    new Error(
      'Failed to execute brace completion provider: invalid brace completion result: braceCompletion must be of type boolean but is undefined',
    ),
  )
})
...
test('executeBraceCompletionProvider - error - definition must be of type object but is function', async () => {
  ...
  await expect(ExtensionHostBraceCompletion.executeBraceCompletionProvider(1, 0)).rejects.toThrow(
    new Error(
      'Failed to execute brace completion provider: invalid brace completion result: braceCompletion must be of type boolean but is () => { }',
    ),
  )
})
